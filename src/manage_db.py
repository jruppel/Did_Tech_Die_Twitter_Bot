...
if not inspector.has_table('games'):
    ...
    games = db.Table('games', metadata, autoload_with=engine)
...
def get_game_data(game_num,sport,date,time,opponent,at,result,team_record,opponent_record,id):
    ...
    with engine.begin() as connection:
        all_games=connection.execute(db.select([games]).where(db.and_(*conditions_list))).fetchall()
    return all_games
    
def get_all_game_data():
    with engine.begin() as connection:
        return connection.execute(db.select(games)).fetchall()
...
def insert_new_game_data(gd_game_num,gd_sport,gd_date,gd_time,gd_opponent,gd_at,gd_result,gd_team_record,gd_opponent_record,gd_id):
    ...
    with engine.begin() as connection:
        connection.execute(db.insert(games).values(game_num=gd_game_num,sport=gd_sport,date=gd_date,time=gd_time,opponent=gd_opponent,at=gd_at,result=gd_result,team_record=gd_team_record,opponent_record=gd_opponent_record,id=gd_id))
    logging.info("New game data inserted!")

def delete_incorrect_game_data(gd_id):
    with engine.begin() as connection:
        connection.execute(db.delete(games).where(games.columns.id==gd_id))
